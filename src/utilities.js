...
class UtilityC {
  constructor({GPIO, UtilityNumber, Description, Details, testFlag, pwmMode, interval = 100, duty = 0.1}) {
    ...
    Object.defineProperty(this, 'Interval', {
      enumerable: true,
      get: () => {
        return new ui.ShowUser({value: this.hiddenInterval, type: ['output', 'number']})
      },
      set: val => {
    ...
    Object.defineProperty(this, 'Duty', {
      enumerable: true,
      get: () => {
        return new ui.ShowUser({value: this.hiddenDuty, type: ['output', 'number']})
      },
      set: val => {
        if (val >= 0 && val <= 1) {
          this.hiddenDuty = val
...
var utilityMap = {
  1: new UtilityC({GPIO: 3, UtilityNumber: 1, Description: 'Heater Relay', Details: '', State: 0}),
}
    ...
    return new Promise(resolve => {
      ...
      if (bkup.configExists(utilitiesPath)) {
        ...
        Object.entries(utilityMap).forEach(([key, value]) => {
          var pinMapIndex = pinMap.getIndexFromGPIO(utilityMap[key].GPIO.value)
          value.Details.value = 'GPIO ' + utilityMap[key].GPIO.value + ' Header: ' + pinMap.HeaderNumber[pinMapIndex] + ' Info: ' + pinMap.Name[pinMapIndex]
          // console.log(value)
